...
import 'package:traffic_department/screens/citizen/services/vehicle_conversion/vehicle_conversion_form_step.dart';
import 'package:traffic_department/screens/citizen/services/vehicle_conversion/vehicle_conversion_upload_documents_step.dart';
import 'package:traffic_department/screens/citizen/services/vehicle_conversion/vehicle_conversion_summary_step.dart';
...
const VehicleConversionScreen({super.key, this.prefilledData});
  ...
  State<VehicleConversionScreen> createState() =>
      _VehicleConversionScreenState();
}

class _VehicleConversionScreenState extends State<VehicleConversionScreen> {
  int _currentStep = 0;
  List<bool> stepCompleted = [false, false, false, false];

  final GlobalKey<VehicleConversionFormStepState> _formStepKey = GlobalKey();
  final GlobalKey<VehicleConversionUploadDocumentsStepState> _uploadDocsKey =
      GlobalKey();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('vehicle_conversion'.tr()),
        centerTitle: true,
      ...
      body: Theme(
        ...
        child: Stepper(
          ...
          steps: [
            Step(
              title: Text(
                'fill_vehicle_conversion_form'.tr(),
            ...
            Step(
              title: Text(
                'vehicle_conversion_summary'.tr(),
                style: TextStyle(color: _getStepColor(2)),
              ),
              content: VehicleConversionSummaryStep(
                formData: formData,
